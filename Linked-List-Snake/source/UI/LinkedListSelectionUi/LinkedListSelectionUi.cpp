...
namespace UI
{
	namespace LinkedListUI {
		...
		void LinkedListSelectionUIController::initializeBackgroundImage()
		{
			...
			background_image->initialize(Config::background_texture_path, gameWindow->getSize().x, gameWindow->getSize().y, sf::Vector2f(0, 0));
		...
		void LinkedListSelectionUIController::singleLinkedListButtonCallback()
		{
			...
			ServiceLocator::getInstance()->GetLevelServices()->createLevel(LinkedListType::SingleLinkedList);
		}
		void LinkedListSelectionUIController::doubleLinkedListButtonCallback()
		{
			...
			ServiceLocator::getInstance()->getSoundService()->playSound(SoundType::BUTTON_CLICK);
			GameService::setGameState(GameState::GAMEPLAY);
			ServiceLocator::getInstance()->GetLevelServices()->createLevel(LinkedListType::DoubleLinkedList);
		}
